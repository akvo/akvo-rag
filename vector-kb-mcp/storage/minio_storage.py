...
logger = logging.getLogger("vector-kb-mcp.storage")


class MinioStorageService:
    """
    MinIO S3-compatible Object Storage service for document files.
    Provides bucket management, streaming uploads, downloads,
    ...
    def __init__(
        ...
        self.default_bucket = self.settings.MINIO_BUCKET_DOCUMENTS
        if client_override:
    ...
    def object_exists(
        ...
        target = bucket_name or self.default_bucket
        try:
            self.client.stat_object(target, object_name)
            return True
        except Exception:
            return False
