...
class RagChatUtil:
    ...
    async def get_knowledge_base_by_label(self, label: str) -> Optional[Dict[str, Any]]:
        """Get a knowledge base by its label.
        
        Args:
            label: Label of the knowledge base
        ...
        self._log("get_knowledge_base_by_label", {"label": label}, {"total_kbs": len(kbs)})
        
        for kb in kbs:
            if kb.get("label") == label:
                self._log("get_knowledge_base_by_label", {"label": label}, {"status": "found", "kb_id": kb.get("id")})
                return kb
        
        self._log("get_knowledge_base_by_label", {"label": label}, {"status": "not_found"})
        return None
    ...
    async def create_chat(self, kb_ids: List[int]) -> Optional[Dict[str, Any]]:
        ...
        chat_url = f"{self.base_url}/api/chats"
    ...
    async def send_message(self, chat_id: int, message: str) -> AsyncGenerator[Tuple[str, Dict[str, Any]], None]:
        ...
        message_url = f"{self.base_url}/api/chats/{chat_id}/messages"
    ...
    async def generate_rag_response(self, query: str, kb_label: str) -> Dict[str, Any]:
        """Generate a RAG response for evaluation
        
        Args:
            query: The query to send
            kb_label: The label of the knowledge base to use
            
        Returns:
            Dictionary with query, response, and retrieval context
        """
        # Get KB by label
        kb = await self.get_knowledge_base_by_label(kb_label)
        if not kb:
            return {
                "query": query,
                "response": f"Error: Knowledge base '{kb_label}' not found",
                "contexts": [],
                "error": f"Knowledge base '{kb_label}' not found"
            }
