...
def detect_csv_format(df: pd.DataFrame) -> Tuple[bool, bool, Optional[str]]:
    ...
    if not prompt_col:
        return (
            False,
            False,
            f"CSV must contain one of these columns: {', '.join(prompt_columns)}",  # noqa
        )
    ...
    if has_references:
        non_empty_refs = (
            df[reference_col].dropna().str.strip().astype(bool).sum()
        )
        if non_empty_refs == 0:
...
def parse_csv_queries(
    ...
    if has_references:
        ...
        if reference_col:
            reference_answers = (
                df[reference_col].fillna("").str.strip().tolist()
            )
            # Ensure reference_answers matches queries length
            if len(reference_answers) != len(df):
                # Pad or truncate to match DataFrame length, then filter to
                # match queries
            ...
            for i, query in enumerate(df[prompt_col]):
                if pd.notna(query) and str(query).strip():
                    ...
                    filtered_refs.append(
                        reference_answers[ref_idx] if ref_idx >= 0 else ""
                    )
            reference_answers = filtered_refs

    logger.info(
        f"Parsed {len(queries)} queries"
        + (
            f" with {len([r for r in reference_answers if r])} reference answers"  # noqa
...
def validate_queries_and_references(
    queries: List[str], reference_answers: Optional[List[str]] = None
    ...
    empty_queries = [
        i for i, q in enumerate(queries) if not q or not q.strip()
    ]
    if empty_queries:
    ...
    if reference_answers is not None:
        if len(reference_answers) != len(queries):
            return (
                False,
                f"Mismatch: {len(queries)} queries but {len(reference_answers)} reference answers",  # noqa
            )
        ...
        if empty_refs:
            logger.warning(
                f"Empty reference answers found at positions: {empty_refs} "
                "- these queries will use reference-free metrics only"
            )
...
) -> Tuple[bool, List, List[str], Optional[str]]:
    """Set up RAGAS for evaluation and determine available metrics.

    Args:
        enable_reference_metrics: If True, include metrics
        that require reference answers
    ...
    try:
        ...
        if ragas_spec is None:
            return (
                ...
                [],
                [],
                "RAGAS package is not installed. Install with 'pip install ragas'",  # noqa
            )
        ...
        # Use LLMContextPrecisionWithoutReference instead of standard
        # ContextPrecision
        try:
            ...
            logger.info(
                "LLMContextPrecisionWithoutReference metric not available"
            )
        ...
        if enable_reference_metrics:
            ...
            try:
                ...
                try:
                    ...
                    metric_names.append(
                        "answer_similarity"
                    )  # Keep same name for UI consistency
                    logger.info(
                        "Added SemanticSimilarity metric as fallback for answer_similarity"  # noqa
                    )
                except ImportError:
                    logger.info(
                        "Neither AnswerSimilarity nor SemanticSimilarity metric available"  # noqa
                    )
...
async def generate_rag_responses(
    ...
    Args:
        ...
        reference_answers: Optional list of reference
        answers (must match queries length if provided)
    ...
    with monitor.measure_operation(
        ...
        chat_util = RagChatUtil(
            base_url=rag_api_url, username=username, password=password
        )
        ...
        if use_batch_processing and len(queries) > 1:
            logger.info(
                f"Using batch processing: {len(queries)} queries, "
                f"batch_size={batch_size}, max_concurrent={max_concurrent}"
            )

            # Use batch processing for better performance
            with monitor.measure_operation(
                "rag_batch_processing", query_count=len(queries)
            ):
            ...
            for i, query in enumerate(queries):
                ...
                with monitor.measure_operation(
                    "rag_api_single_query",
                    query_index=i + 1,
                    query_preview=query[:50],
                ):
                    rag_result = await chat_util.generate_rag_response(
                        query, kb_name
                    )
        ...
        for i, rag_result in enumerate(raw_results):
            ...
            if "error" not in rag_result:
                contexts = []
                for item in rag_result.get("contexts", []):
...
def prepare_evaluation_data(
    ...
    try:
        ...
        # Add 'reference' column for reference-based metrics from
        # reference_answer or ground_truths
        ...
        if "retrieved_contexts" in eval_df.columns:
            context_counts = eval_df["retrieved_contexts"].apply(
            ...
            logger.info(
                f"Context analysis: {rows_with_contexts}/{len(eval_df)} "
                f"rows have contexts, total contexts: {total_contexts}"
            )

            if not has_contexts:
                logger.warning(
                    "No retrieved contexts found in any rows - "
                    "context-based metrics will be skipped"
                )
        else:
        ...
        if "reference" in eval_df.columns:
            ...
            logger.info(
                f"Reference column analysis: {non_empty_refs}/{len(eval_df)} rows have non-empty references"  # noqa
            )
...
def evaluate_faithfulness(
    ...
    with monitor.measure_operation(
        "ragas_eval_faithfulness", dataset_size=len(eval_dataset)
    ):
        try:
            from ragas.metrics import Faithfulness
            ...
            monitor.increment_counter(
                "openai_api_calls"
            )  # This will make multiple API calls
            result = evaluate(
                dataset=eval_dataset, metrics=[faithfulness_metric]
            )

            # Debug: log what we actually got back
            logger.info(f"RAGAS result type: {type(result).__name__}")
            logger.info(
                f"RAGAS result attributes: {[attr for attr in dir(result) if not attr.startswith('_')]}"  # noqa
            )
            logger.info(
                f"RAGAS scores: {result.scores if hasattr(result, 'scores') else 'No scores attr'}"  # noqa
            )

            # Try accessing through scores attribute first (correct for RAGAS
            # 0.2.x)
            if (
                hasattr(result, "scores")
                and isinstance(result.scores, list)
                and len(result.scores) > 0
            ):
                # Extract faithfulness scores from list of dicts
                faithfulness_scores = [
                    score_dict.get("faithfulness")
                    for score_dict in result.scores
                    if "faithfulness" in score_dict
                ]
                if faithfulness_scores:
                    logger.info(
                        f"Faithfulness evaluation completed via scores: {len(faithfulness_scores)} scores"  # noqa
                    )
                    return faithfulness_scores, None
            elif hasattr(result, "faithfulness"):
                faithfulness_scores = result.faithfulness
                logger.info(
                    f"Faithfulness evaluation completed: {type(faithfulness_scores).__name__}"  # noqa
                )
                return (
                    faithfulness_scores.tolist()
                    if hasattr(faithfulness_scores, "tolist")
                    else faithfulness_scores
                ), None
            else:
                logger.warning(
                    "No faithfulness result found in evaluation output"
                )
                # Try to log what's actually in the result
                if hasattr(result, "__dict__"):
                    logger.info(
                        f"Result dict keys: {list(result.__dict__.keys())}"
                    )
                if hasattr(result, "scores") and hasattr(
                    result.scores, "__dict__"
                ):
                    logger.info(
                        f"Scores dict keys: {list(result.scores.__dict__.keys())}"  # noqa
                    )
...
def evaluate_answer_relevancy(
    ...
    try:
        ...
        result = evaluate(
            dataset=eval_dataset, metrics=[answer_relevancy_metric]
        )
        ...
        if (
            ...
            relevancy_scores = [
                score_dict.get("answer_relevancy")
                for score_dict in result.scores
                if "answer_relevancy" in score_dict
            ]
            if relevancy_scores:
                logger.info(
                    f"Answer relevancy evaluation completed via scores: {len(relevancy_scores)} scores"  # noqa
                )
                return relevancy_scores, None
        elif hasattr(result, "answer_relevancy"):
            relevancy_scores = result.answer_relevancy
            logger.info(
                f"Answer relevancy evaluation completed: {type(relevancy_scores).__name__}"  # noqa
            )
            return (
                relevancy_scores.tolist()
                if hasattr(relevancy_scores, "tolist")
                else relevancy_scores
            ), None
        else:
            logger.warning(
                "No answer_relevancy result found in evaluation output"
            )
            # Try to log what's actually in the result
            if hasattr(result, "__dict__"):
                logger.info(
            ...
            if hasattr(result, "scores") and hasattr(
                ...
                logger.info(
                    f"Scores dict keys: {list(result.scores.__dict__.keys())}"
                )
...
def evaluate_context_precision(
    ...
    try:
        ...
        result = evaluate(
            dataset=eval_dataset, metrics=[context_precision_metric]
        )
        ...
        for attr in possible_attrs:
            if hasattr(result, attr):
                ...
                logger.info(
                    f"Found context precision result under attribute: {attr}"
                )
                break

        if precision_scores is not None:
            logger.info(
                f"Context precision evaluation completed: {type(precision_scores).__name__}"  # noqa
            )
            return (
                precision_scores.tolist()
                if hasattr(precision_scores, "tolist")
                else precision_scores
            ), None
        else:
            ...
            if hasattr(result, "scores") and result.scores:
                ...
                for score_dict in result.scores:
                    for key in score_dict:
                        if (
                            "precision" in key.lower()
                            and "context" in key.lower()
                        ):
                            precision_key = key
                            logger.info(
                                f"Found context precision in scores under key: {key}"  # noqa
                            )
                            break
                    if precision_key:
                        break
                ...
                if precision_key:
                    ...
                    precision_scores = [
                        score_dict.get(precision_key)
                        for score_dict in result.scores
                        if precision_key in score_dict
                    ]
                    return precision_scores, None

            logger.warning(
                "No context_precision_without_reference result found in evaluation output"  # noqa
            )
...
def evaluate_context_relevancy(
    ...
    try:
        ...
        result = evaluate(
            dataset=eval_dataset, metrics=[context_relevancy_metric]
        )
        ...
        for attr in possible_attrs:
            if hasattr(result, attr):
                ...
                logger.info(
                    f"Found context relevancy result under attribute: {attr}"
                )
                break

        if relevancy_scores is not None:
            logger.info(
                f"Context relevancy evaluation completed: {type(relevancy_scores).__name__}"  # noqa
            )
            return (
                relevancy_scores.tolist()
                if hasattr(relevancy_scores, "tolist")
            ...
            if hasattr(result, "scores") and result.scores:
                ...
                for score_dict in result.scores:
                    for key in score_dict:
                        if (
                            "context_relevance" in key.lower()
                            or "relevancy" in key.lower()
                        ):
                            relevancy_key = key
                            logger.info(
                                f"Found context relevancy in scores under key: {key}"  # noqa
                            )
                            break
                    if relevancy_key:
                        break
                ...
                if relevancy_key:
                    # Extract scores from all queries
                    relevancy_scores = [
                        score_dict.get(relevancy_key)
                        for score_dict in result.scores
                        if relevancy_key in score_dict
                    ]
                    return relevancy_scores, None

            logger.warning(
                "No context_relevancy result found in evaluation output"
            )
...
def evaluate_metrics_batch(
    ...
    if available_metrics:
        with monitor.measure_operation(
            "batch_eval_unified_metrics",
            metrics=available_metrics,
            metric_count=len(available_metrics),
        ):
            logger.info(
                f"Unified batch evaluation: {len(available_metrics)} metrics - {available_metrics}"  # noqa
            )
            batch_results, batch_errors = _evaluate_unified_metrics_batch(
                eval_dataset, eval_llm, available_metrics
            )
        ...
        logger.warning(
            "No metrics available for evaluation due to missing data requirements"  # noqa
        )

    return results, successful_metrics, errors


def _evaluate_unified_metrics_batch(
    eval_dataset, eval_llm, metrics: List[str]
    ...
    try:
        ...
        logger.info(
            f"Unified batch evaluating {len(metric_instances)} metrics: {metrics}"  # noqa
        )
        monitor = get_monitor()
        ...
        for metric_name in metrics:
            ...
            if (
                ...
                scores = [
                    score_dict.get(ragas_key)
                    for score_dict in result.scores
                    if ragas_key in score_dict
                ]
                if scores and all(score is not None for score in scores):
                    results[metric_name] = scores
                    logger.info(
                        f"Successfully extracted {len(scores)} scores for {metric_name} (ragas key: {ragas_key})"  # noqa
                    )
                else:
                    errors.append(
                        f"{metric_name}: No valid scores found for {ragas_key}"
                    )
                    logger.warning(
                        f"No valid scores for {metric_name} (ragas key: {ragas_key})"  # noqa
                    )
            elif hasattr(result, ragas_key):
                metric_result = getattr(result, ragas_key)
                results[metric_name] = (
                    metric_result.tolist()
                    if hasattr(metric_result, "tolist")
                    else metric_result
                )
                logger.info(
                    f"Successfully extracted attribute result for {metric_name}"  # noqa
                )
            else:
                errors.append(
                    f"{metric_name}: No result found for {ragas_key}"
                )
                logger.warning(
                    f"No result found for {metric_name} (ragas key: {ragas_key})"  # noqa
                )

        logger.info(
            f"Unified batch evaluation completed: {len(results)} successful, {len(errors)} errors"  # noqa
        )
...
def run_ragas_evaluation(
    ...
    Args:
        evaluation_data: List of dictionaries with
        query, answer, contexts, ground_truths
        ...
        If True, enable metrics that require reference answers
        metrics_mode: Metrics evaluation mode
        - 'basic', 'full', or 'reference-only'

    Returns:
        Dictionary with evaluation results or error
    """
    logger.info(
        f"Starting RAGAS evaluation with "
        f"enable_reference_metrics={enable_reference_metrics}, "
        f"metrics_mode={metrics_mode}..."
    )
    ...
    if metrics_mode == "basic":
        ...
        logger.info(
            f"Reference-only mode: evaluating {len(target_metrics)} metrics"
        )
    else:  # metrics_mode == 'full'
        target_metrics = BASIC_METRICS + (
            REFERENCE_METRICS if enable_reference_metrics else []
        )
        logger.info(
            f"Full mode: evaluating {len(target_metrics)} metrics (reference metrics: {enable_reference_metrics})"  # noqa
        )
    ...
    if not api_key:
        return {
            "error": "OpenAI API key not provided. Set OPENAI_API_KEY environment variable."  # noqa
        }
    ...
    logger.info(
        f"DEBUG: setup_ragas returned - available: {ragas_available}, metric_names: {metric_names}"  # noqa
    )
    ...
    try:
        ...
        eval_df, has_contexts, error_msg = prepare_evaluation_data(
            evaluation_data
        )
        if error_msg:
        ...
        if "ground_truths" in eval_df.columns:
            # Check if we have meaningful reference answers (not just empty
            # strings)
            non_empty_refs = eval_df["ground_truths"].apply(
                lambda x: (
                    any(ref.strip() for ref in x)
            ...
            logger.info(
                f"Reference analysis: {ref_count}/{len(eval_df)} rows have reference answers"  # noqa
            )

            if enable_reference_metrics and not has_references:
                logger.warning(
                    "Reference metrics requested but no reference answers found - some metrics will be skipped"  # noqa
                )
        else:
        ...
        try:
            ...
            logger.info(
                f"Created EvaluationDataset with {len(eval_dataset)} samples"
            )
        except Exception as e:
        ...
        logger.info(
            f"RAGAS evaluation completed: {len(successful_metrics)} successful metrics, {len(errors)} errors"  # noqa
        )
...
async def evaluate_queries(
    ...
    Args:
        ...
        reference_answers:
        Optional list of reference answers for enhanced metrics
        metrics_mode: Metrics evaluation mode
        - 'basic', 'full', or 'reference-only'
        ...
        max_concurrent: Maximum concurrent requests per batch
        save_performance_report: Save performance report to JSON file
        (default: False)
    ...
    """
    # Validate inputs
    is_valid, error_msg = validate_queries_and_references(
        queries, reference_answers
    )
    if not is_valid:
        return {"error": error_msg}
    ...
    if metrics_mode == "reference-only":
        ...
        if not reference_answers or not any(
            ref.strip() for ref in reference_answers
        ):
            return {"error": "reference-only mode requires reference answers"}
        enable_reference_metrics = True
        logger.info(
            "DEBUG: Reference-only mode - will evaluate only reference-based metrics"  # noqa
        )
    elif metrics_mode == "basic":
        # Basic mode never uses reference metrics
        enable_reference_metrics = False
        logger.info(
            "DEBUG: Basic mode - will evaluate only basic metrics (no reference required)"  # noqa
        )
    else:  # metrics_mode == 'full'
        # Full mode uses reference metrics if available
        enable_reference_metrics = reference_answers is not None and any(
            ref.strip() for ref in reference_answers
        )
        logger.info(
            f"DEBUG: Full mode - reference metrics enabled: {enable_reference_metrics}"  # noqa
        )

    logger.info(
        f"DEBUG: Metrics mode: {metrics_mode}, enable_reference_metrics: {enable_reference_metrics}"  # noqa
    )
    if reference_answers:
        non_empty = sum(1 for ref in reference_answers if ref.strip())
        logger.info(
            f"DEBUG: Reference answers count: {len(reference_answers)}, "
    ...
    if ragas_status_callback:
        if openai_api_key or os.environ.get("OPENAI_API_KEY"):
            ...
            ragas_status_callback(
                "Skipping RAGAS evaluation: OpenAI API key not provided"
            )

    if openai_api_key or os.environ.get("OPENAI_API_KEY"):
        logger.info(
            f"DEBUG: Calling run_ragas_evaluation with "
            f"enable_reference_metrics={enable_reference_metrics}, "
            f"metrics_mode={metrics_mode}"
        )
        ...
        logger.info(
            f"DEBUG: RAGAS evaluation completed, results keys: {keys_str}"
        )
        ...
        if "success" in ragas_results and ragas_results["success"]:
            ...
            logger.info(
                f"DEBUG: RAGAS success - metrics_data keys: {list(metrics_data.keys())}"  # noqa
            )
            ...
            logger.info(f"DEBUG: Reference metrics in results: {ref_metrics}")
            ...
            for metric, values in metrics_data.items():
                if len(values) != expected_results:
                    logger.warning(
                        f"RAGAS metric '{metric}' only has {len(values)} values for {expected_results} queries"  # noqa
                    )

            for i, result in enumerate(rag_results):
                logger.info(
                    f"DEBUG: Processing result {i+1} - existing keys: {list(result.keys())}"  # noqa
                )
                for metric in metric_names:
                    if metric in metrics_data and i < len(
                        metrics_data[metric]
                    ):
                        result[metric] = metrics_data[metric][i]
                        logger.info(
                            f"DEBUG: Added metric '{metric}' = {metrics_data[metric][i]} to result {i+1}"  # noqa
                        )
                    else:
                        # For missing metrics, assign None to indicate failed
                        # evaluation
                        result[metric] = None
                        logger.warning(
                            f"DEBUG: RAGAS failed to evaluate '{metric}' for query {i+1} - setting to None"  # noqa
                        )
                logger.info(
                    f"DEBUG: Result {i+1} final keys: {list(result.keys())}"
                )
    ...
    if save_performance_report:
        ...
        report_filename = (
            f"performance_reports/evaluation_report_{timestamp}.json"
        )
        try:
...
def run_headless_evaluation(
    ...
    Args:
        ...
        Save performance report to JSON file (default: False)
    ...
    try:
        ...
        if save_performance_report:
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            report_filename = (
                f"performance_reports/evaluation_report_error_{timestamp}.json"
            )
            try:
                monitor.save_report_to_file(
                    report_filename, len(queries) if queries else 0
                )
                logger.info(
                    f"Performance report (error case) saved to: {report_filename}"  # noqa
                )
            except Exception as e:
                logger.warning(
                    f"Failed to save performance report in error case: {str(e)}"  # noqa
                )
...
def convert_numpy_for_json(obj: Any) -> Any:
    ...
    if isinstance(obj, (np.integer, np.int64, np.int32)):
        ...
        return [convert_numpy_for_json(item) for item in obj.tolist()]
    elif isinstance(obj, dict):
        return {
            key: convert_numpy_for_json(value) for key, value in obj.items()
        }
...
def main_cli():
    """
    CLI entrypoint for headless golden evaluation with hard quality gates.
    """
    import argparse
    import json
    import sys
    ...
    parser = argparse.ArgumentParser(
        description="Akvo RAG Headless Golden Set Accuracy Evaluation & Quality Gate Runner"  # noqa
    )
    parser.add_argument(
        "--input-csv",
        "-i",
        ...
        type=str,
        default=None,
        help="Path to evaluation CSV file containing prompts and optional reference answers",  # noqa
    )
    parser.add_argument(
        "--kb",
        "-k",
        ...
        type=str,
        default=None,
        help="Target Knowledge Base name or alias (e.g. 'Kenya Drylands', 'Living Income', 'TDT Library #2')",  # noqa
    )
    parser.add_argument(
        "--min-faithfulness",
        type=float,
        default=0.85,
        help="Target minimum RAGAS faithfulness threshold (default: 0.85)",
    )
    parser.add_argument(
        "--min-relevancy",
        type=float,
        default=0.85,
        help="Target minimum RAGAS answer relevancy threshold (default: 0.85)",
    )
    parser.add_argument(
        "--min-groundedness",
        type=float,
        default=0.90,
        help="Target minimum context precision / groundedness threshold (default: 0.90)",  # noqa
    )
    parser.add_argument(
        "--min-context-recall",
        type=float,
        default=None,
        help="Optional minimum context recall threshold when references exist",
    )
    parser.add_argument(
        "--output-dir",
        "-o",
        dest="output_dir",
        type=str,
        default="/app/RAG_evaluation/performance_reports",
        help="Directory to save JSON and Markdown evaluation performance reports",  # noqa
    )
    parser.add_argument(
        "--api-url",
        "-a",
        ...
        help="URL of the Akvo RAG FastAPI backend (default: http://localhost:8000)",  # noqa
    )
    parser.add_argument(
        "--username",
        "-u",
        type=str,
        default=os.getenv("RAG_USERNAME", "admin@example.com"),
    ...
    parser.add_argument(
        ...
        "-p",
        type=str,
        default=os.getenv("RAG_PASSWORD", "password"),
    ...
    parser.add_argument(
        ...
        help="OpenAI model for RAGAS evaluation (default: gpt-4o)",
    )
    parser.add_argument(
        "--metrics-mode",
        "-m",
        type=str,
        default="full",
        choices=["basic", "full", "reference-only"],
        help="Metrics mode (basic: 4, full: 8, reference-only: 4)",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=5,
        help="Number of queries processed per batch (default: 5)",
    )
    parser.add_argument(
        "--max-concurrent",
        type=int,
        default=3,
        help="Maximum concurrent asynchronous requests (default: 3)",
    )
    ...
    if args.input_csv:
        ...
        try:
            ...
            if error_msg:
                print(f"❌ CSV Parsing Error: {error_msg}")
                sys.exit(1)
            print(
                f"✅ Loaded {len(queries)} queries (References provided: {bool(reference_answers)})"  # noqa
            )

            # Auto-infer KB name if not specified
            if not kb_name:
                csv_lower = os.path.basename(csv_path).lower()
                if "kenya" in csv_lower:
                    kb_name = "Kenya Drylands"
                elif "living" in csv_lower or "li" in csv_lower:
                    kb_name = "Living Income"
                else:
                    kb_name = "Living Income Benchmark Knowledge Base"
                print(
                    f"ℹ️ Inferred Knowledge Base: '{kb_name}' from CSV filename"  # noqa
                )
        except Exception as e:
        ...
        if not kb_name:
            kb_name = "Living Income Benchmark Knowledge Base"
        print(
            f"ℹ️ No input CSV provided, using default sample queries against KB: '{kb_name}'"  # noqa
        )

    print("\n" + "=" * 80)
    print("🚀 STARTING HEADLESS GOLDEN SET EVALUATION")
    print(f"Knowledge Base:  {kb_name}")
    print(f"API URL:         {args.rag_api_url}")
    print(f"OpenAI Model:    {args.openai_model}")
    print(f"Metrics Mode:    {args.metrics_mode}")
    print(
        f"Quality Gates:   Faithfulness >= {args.min_faithfulness}, "
        f"Relevancy >= {args.min_relevancy}, "
        f"Groundedness >= {args.min_groundedness}"
    )
    print("=" * 80 + "\n")
    ...
    if "error" in results and not results.get("ragas_results"):
        print(f"\n❌ Evaluation Failed with Error: {results.get('error')}")
        sys.exit(1)

    # 3. Analyze RAGAS metrics & compute averages
    ragas_results = results.get("ragas_results", {})
    metrics_dict = (
        ragas_results.get("metrics", {})
        if isinstance(ragas_results, dict)
        else {}
    )
    ...
    for metric_name, values in metrics_dict.items():
        ...
        if valid_vals:
            metric_averages[metric_name] = float(np.mean(valid_vals))
        else:
            metric_averages[metric_name] = 0.0

    # Also compute overall response latency
    avg_latency = results.get("avg_response_time", 0.0)

    # 4. Perform Quality Gate Assertions
    gate_checks = []
    overall_passed = True

    # 4.1 Faithfulness Gate
    faithfulness_score = metric_averages.get("faithfulness")
    if faithfulness_score is not None:
        passed = faithfulness_score >= args.min_faithfulness
        if not passed:
        ...
        gate_checks.append(
            {
                ...
                "target": f">= {args.min_faithfulness:.2f}",
                "passed": passed,
            }
        )

    # 4.2 Answer Relevancy Gate
    relevancy_score = metric_averages.get("answer_relevancy")
    if relevancy_score is not None:
        passed = relevancy_score >= args.min_relevancy
        if not passed:
            overall_passed = False
        gate_checks.append(
            {
                ...
                "target": f">= {args.min_relevancy:.2f}",
                "passed": passed,
            }
        )

    # 4.3 Groundedness / Context Precision Gate
    precision_score = (
        metric_averages.get("context_precision")
        if "context_precision" in metric_averages
        else metric_averages.get(
            "context_precision_without_reference",
            metric_averages.get("context_relevancy"),
        )
    )
    if precision_score is not None:
        passed = precision_score >= args.min_groundedness
        if not passed:
            overall_passed = False
        gate_checks.append(
            {
                ...
                "target": f">= {args.min_groundedness:.2f}",
                "passed": passed,
            }
        )

    # 4.4 Context Recall Gate (if reference exists)
    recall_score = metric_averages.get("context_recall")
    if recall_score is not None:
        if args.min_context_recall is not None:
            passed = recall_score >= args.min_context_recall
            if not passed:
                overall_passed = False
            gate_checks.append(
                {
                    ...
                    "score": recall_score,
                    "target": f">= {args.min_context_recall:.2f}",
                    "passed": passed,
                }
            )
        else:
    ...
    # 4.5 Informational metrics
    for other_m in ["answer_similarity", "answer_correctness"]:
        if other_m in metric_averages:
            gate_checks.append(
                {
                    "metric": other_m.replace("_", " ").title(),
                    "score": metric_averages[other_m],
                    "target": "N/A",
                    "passed": True,
                    "info_only": True,
                }
    ...
    # If no RAGAS metrics were evaluated
    if not gate_checks:
        print(
            "⚠️ Warning: No RAGAS metrics could be evaluated (Check OpenAI API key and dataset format)."  # noqa
        )
        overall_passed = False

    # 5. Print Summary Table
    print("\n" + "=" * 80)
    print("📊 RAGAS GOLDEN SET EVALUATION QUALITY GATE SUMMARY")
    print("=" * 80)
    print(f"Knowledge Base:     {kb_name}")
    print(
        f"Total Queries:      {len(queries) if queries else len(DEFAULT_TEST_QUERIES)}"  # noqa
    )
    print(f"Average Latency:    {avg_latency:.2f}s per query")
    print("-" * 80)
    print(f"{'Metric':<35} | {'Score':<10} | {'Target':<12} | {'Status':<10}")
    print("-" * 80)
    for check in gate_checks:
        status_str = (
            "ℹ️ INFO"
            if check.get("info_only")
            else ("✅ PASS" if check["passed"] else "❌ FAIL")
        )
        score_str = f"{check['score']:.4f}"
        print(
            f"{check['metric']:<35} | {score_str:<10} | {check['target']:<12} | {status_str:<10}"  # noqa
        )
    print("=" * 80)

    if overall_passed:
        print("🎉 QUALITY GATE RESULT: ALL GOLDEN SET METRICS PASSED (EXIT 0)")
    else:
        print(
            "❌ QUALITY GATE RESULT: METRICS FELL BELOW TARGET THRESHOLD (EXIT 1)"  # noqa
        )
    print("=" * 80 + "\n")

    # 6. Save Reports to Output Directory
    os.makedirs(args.output_dir, exist_ok=True)
    timestamp_str = datetime.now().strftime("%Y%m%d_%H%M%S")
    json_path = os.path.join(
        args.output_dir, f"golden_evaluation_report_{timestamp_str}.json"
    )
    md_path = os.path.join(
        args.output_dir, f"golden_evaluation_report_{timestamp_str}.md"
    )
    latest_json_path = os.path.join(
        args.output_dir, "latest_evaluation_report.json"
    )
    latest_md_path = os.path.join(
        args.output_dir, "latest_evaluation_report.md"
    )

    report_payload = {
        "timestamp": datetime.now().isoformat(),
        "kb_name": kb_name,
        "dataset": args.input_csv or "default_queries",
        "total_queries": (
            len(queries) if queries else len(DEFAULT_TEST_QUERIES)
        ),
        "avg_latency_seconds": avg_latency,
        "overall_passed": overall_passed,
        "metric_averages": metric_averages,
        "gate_checks": gate_checks,
        "results": results.get("rag_results", []),
    }

    serializable_payload = convert_numpy_for_json(report_payload)
    ...
    try:
        md_content = f"""# RAG Golden Set Evaluation Quality Gate Report

- **Date:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')}
- **Knowledge Base:** `{kb_name}`
- **Dataset:** `{args.input_csv or 'default_queries'}`
- **Total Queries:** `{len(queries) if queries else len(DEFAULT_TEST_QUERIES)}`
- **Average Latency:** `{avg_latency:.2f}s`
- **Quality Gate Status:** **{'PASSED ✅' if overall_passed else 'FAILED ❌'}**
...
"""
        for check in gate_checks:
            ...
            md_content += f"| **{check['metric']}** | `{check['score']:.4f}` | `{check['target']}` | {status_badge} |\n"  # noqa

        pass_f = (
            "PASS"
            if faithfulness_score
            and faithfulness_score >= args.min_faithfulness
            else "FAIL"
        )
        pass_r = (
            "PASS"
            if relevancy_score and relevancy_score >= args.min_relevancy
            else "FAIL"
        )
        pass_g = (
            "PASS"
            if precision_score and precision_score >= args.min_groundedness
            else "FAIL"
        )
...
## 2. Executive Summary
- **Faithfulness Target ($\ge {args.min_faithfulness}$):** `{pass_f}`
- **Answer Relevancy Target ($\ge {args.min_relevancy}$):** `{pass_r}`
- **Context Groundedness Target ($\ge {args.min_groundedness}$):** `{pass_g}`
