...
kb_label = st.sidebar.text_input("Knowledge Base Label", "Living Income Benchmark Knowledge Base")
st.session_state.selected_kb = kb_label
...
async def run_evaluation(queries: List[str], kb_label: str):
    ...
    for i, query in enumerate(queries):
        ...
        rag_result = await chat_util.generate_rag_response(query, kb_label)
        end_time = time.time()
...
if st.button("Run Evaluation", disabled=st.session_state.evaluation_running):
    asyncio.run(run_evaluation(queries, kb_label))
